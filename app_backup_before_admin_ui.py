...
from services.ai_agent import OllamaUnavailableError, ask_ai
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing your utility data..."):
            ...
            if not time_logs.empty:
                database_dataframes.append(time_logs)
            ai_dataframe = (
            ...
            if ai_dataframe.empty:
                ...
                data_context = ai_dataframe.to_string(index=False)
                data_signature = hashlib.sha256(data_context.encode("utf-8")).hexdigest()
                try:
                    answer = get_previous_answer(question, data_signature)
                except Exception:
                    answer = None
                ...
                if not answer:
                    ...
                    if answer:
                        try:
                            save_question_history(question, answer, data_signature)
                        except Exception:
                            pass
...
def load_uploaded_master_data():
    ...
    if csv_energy_rows or csv_solar_rows:
        if csv_energy_rows:
...
def load_dashboard_data():
    """Read the Command Center snapshot from PostgreSQL only."""
    try:
        snapshot = load_dashboard_snapshot()
        snapshot["data_source"] = "PostgreSQL dashboard snapshot"
        return snapshot
    except Exception:
        return {
            "energy": pd.DataFrame(),
...
uploaded_master_data = load_dashboard_data()
...
def fetch_weather_context(location_name="Bangalore"):
...
# -----------------------------
    ...
    st.caption(
        ...
        f"{st.session_state.get('selected_location', 'Bangalore')} / "
        f"{st.session_state.get('selected_plant', 'TPREL-Bangalore')} / "
                            ...
                            if workbook_sheets is not None:
                                try:
                                    ...
                                    solar_records = ingest_uploaded_file(
                                        file,
                                        st.session_state.get("selected_location", "Bangalore"),
                                        st.session_state.get("selected_plant", "TPREL-Bangalore"),
    ...
    render_excel_uploader(
        selected_location=st.session_state.get("selected_location", "Bangalore"),
        selected_plant=st.session_state.get("selected_plant", "TPREL-Bangalore"),
